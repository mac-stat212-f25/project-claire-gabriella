---
title: "iMessage SQL Project"
author: "Claire Kuno and Gabriella Nieves"
...
abstract: "This is an exploratory analysis of our iMessage datbase using SQL query's to scrape the information and tidyverse to visualize emotional patterns and time patterns. Our analysis of our database concluded in trends of positive words being more frequently used in the past years compared to years before and words of anticipation being the most common emotion of words. The most frequent time messages were sent was 3PM-2AM. Overall, texts of most frequency have to with making plans. Future work includes joining in more iMessage data and more detailed emotional lexicon."
editor: 
  markdown: 
    wrap: sentence
---
...
df_joined <- df %>%
  left_join(tfidf %>% group_by(id) %>% summarise(tfidf = pick(everything())),
            by = "id")
df_tidy <- df %>% left_join(tfidf, by = "id")
```

## Most Frequently Used Word Cloud
...
## Positive or Negative Messages in Past Two Years
...
emotion_dist <- df_tidy %>%
  inner_join(get_sentiments("nrc"), by = "word", relationship =
  "many-to-many") %>%
...
## How Often are We Texting? And at What Time?
